...
/**
 * （缓存 Ehcache）分录  业务操作service 基础接口类 <p>
 * @author LingMin 
 * @date 2015-12-02<br>
 * @version 1.0<br>
 */
public abstract class BaseEntryEhcacheServiceImpl<E extends AbstractBaseDataEntry, ID extends java.io.Serializable>
		extends CoreBaseServiceImpl<E, ID> implements IBaseEntryService<E, ID> {
	 ...
	 * @param baseEntryDao
	 */
	public BaseEntryEhcacheServiceImpl(IBaseEntryDao<E, ID> baseEntryDao) {
		super(baseEntryDao);
		this.baseEntryDao = baseEntryDao;
	}

	/**
	 * 根据主表关联字段名与主表关键字获取满足条件的数据关键字
	 * <p>
	 */
	@Cacheable(value = "xbmtCache", key = "#parentField + #parentKey")
