 ...
 * @version 1.0<br>
 */
public class HxdlTest {
	
	//单例模式
	public final static HxdlTest HxdlInstance = new HxdlTest();
	...
	//
	private volatile AtomicInteger currentIndex2 = new AtomicInteger(0);
	 ...
	 * HxdlTest
	 */
	public static HxdlTest getInstance(){
		return HxdlInstance;
	}
	 ...
	 * HxdlTest
	 */
	public static HxdlTest getInstance(int length){
		return HxdlInstance;
	}
	/***
	 * 初始化环形队列数组<p>
	...
	public synchronized void putUid(String uid){
	  ...
	  */
		...
		System.out.println("thread.name="+Thread.currentThread().getName()+"\t putUid.this.currentIndex="+this.currentIndex);
		//首先判断 当前uid是否存在
		if(this.maps.containsKey(uid)){
			...
			//将uid重新加入到新的slot中
			this.maps.put(uid, this.currentIndex);
		}else{
			//设置用户id  所对应 环形队列索引位置
			this.maps.put(uid, this.currentIndex);
			CusQueue temp =  this.cusQueueList[this.currentIndex];
	...
	private  void startProces(){
		...
		System.out.println("startProces.currentIndex="+currentIndex+" \t this.previousIndex="+previousIndex);
		
		//排除当前刚刚增加的元素
		if(this.previousIndex != this.currentIndex){
			CusQueue temp =  this.cusQueueList[this.currentIndex];
			//显示超时的用户列表
			this.displayTaskSets(currentIndex , temp.getTaskSets());
		}else{
	...
	private  void displayTaskSets(int index , Set<String> taskSets){
		if(taskSets == null || taskSets.isEmpty()){
			System.out.println("currentIndex="+currentIndex+" \t taskSets is Empty");
			return;
		}
		System.out.print("currentIndex="+currentIndex+" \t taskSets is "+taskSets.size());
	...
	/**
	 * 获取currentIndex<p>
	 * @return  currentIndex  currentIndex<br>
	 */
	...
	/**
	 * 设置currentIndex<p>
	 * @param  currentIndex  currentIndex<br>
	 */
