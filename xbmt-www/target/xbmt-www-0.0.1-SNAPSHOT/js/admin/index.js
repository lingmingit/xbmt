...
function addPanelURL(){
	var url = "/xbmt-www/xbmtadmin/console/test";
	index++;
	$('#mainTabs').tabs('add',{
		title: 'Tab'+index,
		href: url,
...
function addUserTab(){
	var url = "/xbmt-www/xbmtadmin/console/sys/user/index";
	addMainTab('1' , '用户管理' , url , "icon-ok");
}
function addModuleTab(){
	var url = "/xbmt-www/xbmtadmin/console/sys/modules/index";
	addMainTab('2' , '模块管理' , url );
}


function addMenuTab(){
	var url = "/xbmt-www/xbmtadmin/console/sys/menu/index";
	addMainTab('3' , '菜单管理' , url );
}

function addFunctionActionTab(){
	var url = "/xbmt-www/xbmtadmin/console/sys/functionAction/index";
